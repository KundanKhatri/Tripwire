"""Unit tests for L1 pattern bank — no Azure calls (shields + corpus stubbed)."""
...
def l1() -> L1SemanticFirewall:
    return L1SemanticFirewall(shields=_StubShields(), corpus=_StubCorpus())
...
async def test_blocks_known_injections(l1: L1SemanticFirewall, payload: str) -> None:
    ...
    assert result.signals["pattern_hits"]
